...
switch($_GET[act]){
  ...
  if (empty($offset)) {
    ...
    $seleksi = "SELECT * FROM orders,kustomer WHERE orders.id_kustomer=kustomer.id_kustomer ORDER BY id_orders DESC LIMIT $offset, $limit";
    ...
    if ($jumlah%$limit) {
      ...
      for ($i=1;$i<=$halaman;$i++) {
        $newoffset=$limit*($i-1);
          if ($offset!=$newoffset)
            {
            echo "<b><font face=\"arial\" size=\"3\"><a style=\"color: black\"  href=\"media.php?module=order&offset=$newoffset\">$i</a>&nbsp;&nbsp;</font></B>";
          } else {
            echo "<font face=\"arial\" size=\"3\" color=\"blue\">[$i]</font>";
          }
      }
            echo "</div>";
    while($r=mysql_fetch_array($result)){
      ...
      echo "<tr><td align=center>$r[id_orders]</td>
                ...
                <td>$r[status_order]</td>
		            <td><a href=?module=order&act=detailorder&id=$r[id_orders]><img src='images/icn_baca.png' title='detail'></a> |
					 <a href=$aksi?module=order&act=hapus&id=$r[id_orders]><img src='images/icn_trash.png' title='hapus'></a>
             </td></tr>";
      $no++;
    }
    echo "</table>";
