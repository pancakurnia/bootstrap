...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Pemesanan Barang</h2>
          <table>
          <tr><th>no</th><th>nama</th><th>email</th><th>subjek</th><th>tanggal</th><th>Status</th> <th> Harga Barang </th><th>aksi</th></tr>";


    $tampil=mysql_query("SELECT * FROM hubungi,kustomer WHERE hubungi.id_kustomer=kustomer.id_kustomer ORDER BY id_hubungi DESC LIMIT 9");
    ...
    while ($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
				...
				<td>$r[status]</td>
				<td>$r[harga]</td>
                <td><a href=$aksi?module=hubungi&act=hapus&id=$r[id_hubungi]><img src='images/icn_trash.png' title='Hapus'></a> | 
				<a href=?module=hubungi&act=view&id=$r[id_hubungi]><img src='images/icn_baca.png' title='View'></a>
                </td></tr>";
    $no++;
    }
    echo "</table>";
